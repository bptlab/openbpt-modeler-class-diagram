...
export default class CustomRuleProvider extends RuleProvider {
  ...
  init() {
    ...
    this.addRule("connection.start", function (context) {
      ...
      return (
        is(source, `${MODELER_PREFIX}:Node`) && {
          type: `${MODELER_PREFIX}:Association`,
        }
      );
    });
    ...
    this.addRule("shape.resize", function (context) {
      if (context.shape.type === `${MODELER_PREFIX}:Class`) {
        return true;
      }
      return false;
    });
...
function canConnect(source, target) {
  return (
    source.type === `${MODELER_PREFIX}:Class` &&
    target.type === `${MODELER_PREFIX}:Class` && {
      type: `${MODELER_PREFIX}:Association`,
    }
  );
}
...
function canDrop(shape, target) {
  ...
  // ModelElements cannot be dropped on other ModelElements
