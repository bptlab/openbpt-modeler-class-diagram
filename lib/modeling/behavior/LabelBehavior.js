...
export default class LabelBehavior extends CommandInterceptor {
  constructor(eventBus, modeling, customElementFactory, textRenderer) {
    ...
    this.postExecute("element.updateProperties", function (e) {
      ...
      if (is(element, `${MODELER_PREFIX}:Association`)) {
        if ("associationType" in properties) {
          ...
          const labelAttribute = "targetMultiplicity";
          ...
          if (
            ...
            // If we switch away from inheritance, we need to (re-)create the targetMultiplicity label
            const labelIdToCreate =
              businessObject.id + "_label" + "_" + labelAttribute;
            if (!element.labels.find((l) => l.id === labelIdToCreate)) {
              businessObject.labelAttribute = labelAttribute;

              const labelCenter = getTargetLabelPosition(element);
            ...
            properties["associationType"] === ASSOCIATION_TYPES.INHERITANCE
          ) {
            // If we switch to inheritance, we need to remove the targetMultiplicity label
            const labelIdToRemove =
              businessObject.id + "_label" + "_" + labelAttribute;
    ...
    this.postExecute(["shape.create", "connection.create"], function (e) {
      ...
      const sourceLabelPosition = getSourceLabelPosition(element);
      const targetLabelPosition = getTargetLabelPosition(element);
      ...
      if (is(element, `${MODELER_PREFIX}:Association`)) {
        for (let labelAttribute of [
          ...
          if (labelAttribute === "sourceMultiplicity") {
            ...
            labelCenter = targetLabelPosition;
          }
          businessObject.labelAttribute = labelAttribute;
          modeling.createLabel(element, labelCenter, {
            id: businessObject.id + "_label" + "_" + labelAttribute,
