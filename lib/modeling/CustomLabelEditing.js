...
export default class CustomLabelEditing {
  constructor(eventBus, canvas, directEditing, commandStack, textRenderer) {
    ...
    eventBus.on("element.dblclick", (event) => {
      ...
      if (is(element, `${MODELER_PREFIX}:Class`)) {
        this.detectClassSectionClicked(event);
      } else if (
        is(element, `${MODELER_PREFIX}:Association`) &&
        !isLabel(element)
      ) {
        // Handle case when there is no association label yet (besides multiplicity labels)
        // Setting the labelAttribute ensures that a new label is created
  ...
  detectClassSectionClicked = function (event) {
    ...
    const dividerY =
      (event.element.y + 30 - this._canvas._cachedViewbox.y) * zoom;
  ...
  getEditingBBox(element) {
    ...
    if (element.type === `${MODELER_PREFIX}:Class`) {
      ...
      if (element.businessObject.labelAttribute === "attributes") {
        assign(bounds, {
          y: bbox.y + 30 * zoom,
