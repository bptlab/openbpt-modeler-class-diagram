...
export default class CustomLabelEditing {
      ...
      openAttributeEditor({
        ...
        onSave: (newAttrs) => {
          ...
          const attrs = newAttrs
            // filter out empty attributes (both name and type are empty)
            .filter(
              (attr) => attr.name.trim() !== "" || attr.type.trim() !== "",
            )
            // map to serializable format
            .map((attr) => {
              const attrElement = this._customElementFactory.create(
              ...
              attrElement.$parent = event.element;
              return attrElement;
            });
  ...
  getEditingBBox(element) {
    ...
    if (element.type === `${MODELER_PREFIX}:Class`) {
      ...
      if (getBusinessObject(element).expanded) {
        assign(bounds, {
