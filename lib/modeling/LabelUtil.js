...
export function getLabel(element, labelAttribute = null) {
  const businessObject = getBusinessObject(element);
  const attr = labelAttribute || getLabelAttribute(element);
  if (attr) {
...
export function getLabelAttribute(element) {
  ...
  } else if (businessObject.labelAttribute) {
    return businessObject.labelAttribute;
  } else if (isAny(businessObject, [`${MODELER_PREFIX}:Class`])) {
    return "name";
  }
}

export function setLabel(element, text, labelAttribute = null) {
...
export function requiresExternalLabel(element) {
  const businessObject = getBusinessObject(element);
  return isAny(businessObject, [`${MODELER_PREFIX}:Association`]);
}

export function buildAssociationLabelId(associationId, labelAttribute) {
  return associationId + "_label_" + labelAttribute;
}
