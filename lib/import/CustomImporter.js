...
import {
  getSourceLabelCenterPosition,
  getTargetLabelCenterPosition,
...
export default class CustomImporter {
  ...
  add(semantic, parentElement) {
    ...
    // (optional) LABEL
    if (requiresExternalLabel(semantic)) {
      if (is(semantic, `${MODELER_PREFIX}:Association`)) {
        ...
        labelAttributes.forEach((labelAttribute) => {
          if (semantic[labelAttribute]) {
            this.addLabel(semantic, element, labelAttribute);
          }
        });
      }
    }
   ...
   */
  addLabel(semantic, element, labelAttribute) {
    const text = semantic[labelAttribute];
    ...
    if (labelAttribute === "sourceMultiplicity") {
      const sourceLabelCenterPosition = getSourceLabelCenterPosition(element);
      // Set starting coordinates of label based on its center position
      bounds.x = sourceLabelCenterPosition.x - bounds.width / 2;
      bounds.y = sourceLabelCenterPosition.y - bounds.height / 2;
    } else if (labelAttribute === "targetMultiplicity") {
      const targetLabelCenterPosition = getTargetLabelCenterPosition(element);
      // Set starting coordinates of label based on its center position
      bounds.x = targetLabelCenterPosition.x - bounds.width / 2;
      bounds.y = targetLabelCenterPosition.y - bounds.height / 2;
    }
