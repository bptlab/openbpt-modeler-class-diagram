...
export default class CustomRenderer extends BaseRenderer {
  ...
  drawClass = function (parentGfx, element, attrs) {
    ...
    const titleSectionEnd = this.renderTitleSection(parentGfx, element);
  ...
  renderTitleSection = function (parentGfx, element) {
    ...
    if (text) {
      const padding = 5;
      const label = this.renderLabel(parentGfx, text || "", {
        // Do not set height here, let it adapt automatically
        box: {
          width: element.width,
        },
        padding,
  ...
  renderAttributeSection = function (parentGfx, element, offsetY) {
    ...
    if (businessObject.attributes && businessObject.attributes.length > 0) {
      const attributeSectionOffsetY = offsetY;
      const attributeStrings = businessObject.attributes.map(
        (attr) =>
          `${visibilityToStringMap[attr.visibility] || ""} ${attr.name}: ${attr.type}`,
      );
      const text = attributeStrings.join("\n");
  ...
  drawAssociation = function (parentGfx, element) {
    ...
    if (associationType === "standard") {
      ...
      let markerEnd;
      if (associationType === ASSOCIATION_TYPES.INHERITANCE) {
        markerEnd = this.marker("inheritance-end", "white", "black");
