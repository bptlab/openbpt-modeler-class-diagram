...
export default class CustomPaletteProvider {
  ...
  getPaletteEntries(element) {
    ...
    assign(actions, {
      ...
      "create-collapsed-class": createAction(
        `${MODELER_PREFIX}:Class`,
        "custom-elements",
        generatedIcons["cd-class-simple"],
        translate("Create collapsed class"),
        { expanded: false },
      ),
      "create-class": createAction(
        `${MODELER_PREFIX}:Class`,
        "custom-elements",
        generatedIcons["cd-class-attributes-only"],
        translate("Create class with Attributes"),
      ),
    });
    return actions;
  }
