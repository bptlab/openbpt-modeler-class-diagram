...
export function getFlowNodePosition(source, element) {
  ...
  const position = {
    ...
    // Somehow, there seems to be a vertical offset of 1px when placing an element on the same y coordinate.
    // This workaround ensures that the new element is placed exactly on the same y coordinate as the source element.
    y: sourceMid.y + getVerticalDistance(orientation, minDistance) - 1,
  };
